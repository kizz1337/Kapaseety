 var gaugeOptions = {
	chart: { type: 'solidgauge', backgroundColor:'#457DBB',borderRadius:5,borderwidth:0,shadow: true},
	title: null,
...
function init() {
	...
	$(".tablesorter").tablesorter({theme: 'bootstrap',headerTemplate: '{content}{icon}',widgets : [ "uitheme"]});
	 ...
	 $('#vm_total .label, #hosts_total .label').hover(function() {
		$(this).css('cursor','pointer');
	});

	$('#topmenu a').unbind();
	$('#topmenu a').click(function(){
		url = '/?m='+$(this).attr('data-href');
		//~ update_url(url);
	...
	$('#hosts_total').click(function(){
		url ='/?m=hosts&name='+$(this).attr('data-href')+'&moref='+$('#moref').html();
		$('#content').load(url,function(){
			init();
		});	
